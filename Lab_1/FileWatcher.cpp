...
FileWatcher* FileWatcher::getInstance()
{
	if (instance == nullptr)
	{
		return instance = new FileWatcher();
	}
...
void FileWatcher::addFile(QString filePath)
{
	...
	if (m_fileList.contains(QFileInfo(filePath))) {
		emit fileAlreadyAddedToWatcher(filePath.toStdString());
	}
	else {
...
{
	if (!m_fileList.contains(QFileInfo(filePath))) {
		emit noSuchFileInWatcher(filePath.toStdString());
	}
	else {
		QFileInfo fileInfo(filePath);
